...
class EnvironmentDbProvider(DbProviderBase):
    ...
    def create_environment_table_if_required(self):
        return self.cursor.execute("""CREATE TABLE IF NOT EXISTS environments(
    ...
    def persist_environment(self, environment, throw_if_exists=True, should_commit=True):

        if self.cursor.execute("SELECT COUNT(*) from environments where envid = ?", (environment.id(), )).fetchone()[0] > 0:
            message = f"An environment with identifier '{environment.id()}' already exists"
            Logger.log(message)
            if throw_if_exists:
