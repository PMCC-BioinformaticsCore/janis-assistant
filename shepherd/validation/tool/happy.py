...
class HapPyValidator(CommandTool):
    ...
    def outputs(self) -> List[ToolOutput]:
        return [
            ToolOutput(
                "extended", Csv(), glob=InputSelector("reportPrefix") + ".extended.csv"
            ),
            ToolOutput(
                "summary", Csv(), glob=InputSelector("reportPrefix") + ".summary.csv"
            ),
            ToolOutput(
                "metrics",
                File(),
                glob=InputSelector("reportPrefix") + ".metrics.json.gz",
            ),
            ToolOutput(
                "vcf", VcfTabix(), glob=InputSelector("reportPrefix") + ".vcf.gz"
            ),
            ToolOutput(
                "runinfo",
                JsonFile(),
                glob=InputSelector("reportPrefix") + ".runinfo.json",
            ),
            ToolOutput(
                "roc", File(), glob=InputSelector("reportPrefix") + ".roc.all.csv.gz"
            ),
            ToolOutput(
                "indelLocations",
                File(),
                glob=InputSelector("reportPrefix") + ".roc.Locations.INDEL.csv.gz",
            ),
            ToolOutput(
                "indelPassLocations",
                File(),
                glob=InputSelector("reportPrefix") + ".roc.Locations.INDEL.PASS.csv.gz",
            ),
            ToolOutput(
                "snpLocations",
                File(),
                glob=InputSelector("reportPrefix") + ".roc.Locations.SNP.csv.gz",
            ),
            ToolOutput(
                "snpPassLocations",
                File(),
                glob=InputSelector("reportPrefix") + ".roc.Locations.SNP.PASS.csv.gz",
            ),
        ]
