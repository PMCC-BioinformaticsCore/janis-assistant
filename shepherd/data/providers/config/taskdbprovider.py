...
class TaskDbProvider(DbProviderBase):

    table_name = "tasks"

    def __init__(self, db, cursor):
        super(TaskDbProvider, self).__init__(db, cursor)

        self.create_tasks_table_if_required()

    def create_tasks_table_if_required(self) -> None:
        self.cursor.execute(f"""CREATE TABLE IF NOT EXISTS {TaskDbProvider.table_name}(
    ...
    def get_by_tid(self, tid) -> Optional[TaskRow]:
        row = self.cursor.execute(f"SELECT * FROM {TaskDbProvider.table_name} WHERE tid = ?", (tid,)).fetchone()
        if len(row) == 0: return None

        return TaskRow.from_row(row[0])

    def get_all_tasks(self) -> [TaskRow]:
        return [TaskRow.from_row(r) for r in self.cursor.execute(f"SELECT * FROM {TaskDbProvider.table_name}").fetchall()]
    ...
    def insert_task(self, task: TaskRow) -> None:
        ...
        self.cursor.execute(f"INSERT INTO {TaskDbProvider.table_name}({str_insfields}) VALUES ({', '.join(str_insplaceholder)})", task.to_row())
        self.commit()
